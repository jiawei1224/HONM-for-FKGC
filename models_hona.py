...
class MetaR(nn.Module):
    def __init__(self, dataset, parameter, num_symbols, embed = None):
        ...
        # #优化1
        self.W_r = nn.Linear(self.embed_dim, 2*self.embed_dim)
        self.W_e = nn.Linear(self.embed_dim, 2*self.embed_dim)
        self.bias = nn.Parameter(torch.zeros(2*self.embed_dim))
        self.gcn_w = nn.Linear(2 * self.embed_dim, self.embed_dim)  # change log
        #
        #
        # #优化1.2
        # self.W_r = nn.Linear(self.embed_dim, self.embed_dim)
        # self.W_e = nn.Linear(self.embed_dim, self.embed_dim)
        # self.bias = nn.Parameter(torch.zeros(self.embed_dim))
        # self.gcn_w = nn.Linear(self.embed_dim, self.embed_dim)  # change log

         #优化3
        self.W_r2 = nn.Linear(self.embed_dim, 2 * self.embed_dim)
        self.W_e2 = nn.Linear(self.embed_dim, 2*self.embed_dim)
        self.bias2 = nn.Parameter(torch.zeros(2*self.embed_dim))
        self.gcn_w2 = nn.Linear(4 * self.embed_dim, self.embed_dim)  # change log
        self.gcn_b2 = nn.Parameter(torch.FloatTensor(self.embed_dim))  # change log
    ...
    def neighbor_encoder(self, connections, connections2,num_neighbors, iseval):
        '''
        connections: (batch, 200, 2)
        num_neighbors: (batch,)
        '''
        num_neighbors = num_neighbors.unsqueeze(1)
        entity_self = connections[:,0,0].squeeze(-1) #头实体
        relations = connections[:,:,1].squeeze(-1) #关系
        entities = connections[:,:,2].squeeze(-1) #尾实体
        entities2 = connections2[:, :, 2].squeeze(-1)  # 2-hop尾实体
        ...
        ent2_embeds = self.dropout(self.symbol_emb(entities2))  # (batch, 200, embed_dim)

        if not iseval:
            entself_embeds = entself_embeds.squeeze(1)

        g = self.sigmoid(self.W_r(rel_embeds) + self.W_e(ent_embeds) + self.bias)
        concat_embeds = g * self.W_r(rel_embeds) + (1 - g) * self.W_e(ent_embeds)



        g1 = self.sigmoid(self.W_r2(rel_embeds) + self.W_e2(ent2_embeds) + self.bias2) #rel_embeds还是concat_embeds
        concat_embeds2 = g1 * self.W_r2(rel_embeds) + (1 - g1) * self.W_e2(ent2_embeds)

        concat_embeds_final = torch.cat((concat_embeds, concat_embeds2), dim=-1)  # (batch, 200, 2*embed_dim)


        out = self.gcn_w2(concat_embeds_final) + self.gcn_b2  # out gcn former change log



        out = F.leaky_relu(out)                            # out gcn former change log 公式（2）
    ...
    def forward(self, task, iseval=False, curr_rel='', support_meta=None, istest=False):
        ...
        support_left_connections, support_left_degrees, support_right_connections, support_right_degrees,support_left_connections2, support_right_connections2 = support_meta[0]
        support_left = self.neighbor_encoder(support_left_connections, support_left_connections2,support_left_degrees, iseval)
        support_right = self.neighbor_encoder(support_right_connections, support_right_connections2, support_right_degrees, iseval)
        ...
        for i in range(self.few-1):
            support_left_connections, support_left_degrees, support_right_connections, support_right_degrees,support_left_connections2, support_right_connections2, = support_meta[i+1]
            support_left = self.neighbor_encoder(support_left_connections, support_left_connections2,support_left_degrees, iseval)
            support_right = self.neighbor_encoder(support_right_connections, support_right_connections2, support_right_degrees,iseval)
