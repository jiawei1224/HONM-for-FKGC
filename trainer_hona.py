...
class Trainer:
    def __init__(self, data_loaders, dataset, parameter):
        ...
        self.max2_neighbor = parameter['max2_neighbor']
        ...
        degrees = self.build_connection(max_=self.max_neighbor,max2_=self.max2_neighbor)
    ...
    def build_connection(self, max_=100, max2_=7):

        self.connections = (np.ones((self.num_ents, max_, 3)) * self.pad_id).astype(int) #68544*100*3
        self.connections2 = (np.ones((self.num_ents, max_, 3)) * self.pad_id).astype(int)  # 68544*100*3

        self.e1_rele2 = defaultdict(list)
        self.e1_degrees = defaultdict(int)
        self.e2_degrees = defaultdict(int)
        self.top_2hop_dict = {}
        ...
        with open(self.data_path + '/path_graph') as f:
            ...
            for line in tqdm(lines):
                ...
                self.e1_rele2[e1].append((self.symbol2id[e1], self.symbol2id[rel], self.symbol2id[e2])) #正向三元组。实体：头实体对应的三元组 68544
                self.e1_rele2[e2].append((self.symbol2id[e2], self.symbol2id[rel + '_inv'], self.symbol2id[e1])) #反向三元组
     #symbol2id和ent2id的区别为：symbol2id为en2id和rel2id的总和
        degrees = {}
        # 先统计每个实体的度数
        degree_dict = {}
        # 遍历所有实体，统计邻居数量
        for ent in self.ent2id.keys():
            ...
            degree_dict[ent] = len(neighbors)

        for ent, id_ in self.ent2id.items(): #遍历所有实体
            neighbors = self.e1_rele2[ent] #ent对应的三元组
            neighbors = sorted(
                neighbors,
                key=lambda x: degree_dict.get(x[2], 0),  # 防止某些邻域实体没度数，默认 0
                reverse=True)
            ...
            for triple in neighbors:
                neigh_ent = triple[2]  # 1-hop邻居实体
                neigh_neighbors = self.e1_rele2.get(neigh_ent, [])
                two_hop_neighbors.extend(neigh_neighbors)

            # 对所有2-hop邻居按度数排序，取前max_2 hop个
            two_hop_neighbors = sorted(two_hop_neighbors, key=lambda x: degree_dict.get(x[2], 0), reverse=True)
            if len(two_hop_neighbors) > max2_:
                two_hop_neighbors = two_hop_neighbors[:max2_]

            # 存2-hop邻居
            for idx, triple in enumerate(two_hop_neighbors):
                self.connections2[id_, idx, 0] = triple[0]
                self.connections2[id_, idx, 1] = triple[1]
                self.connections2[id_, idx, 2] = triple[2]
    ...
    def get_meta(self, left, right):
        ...
        left_connections2 = Variable(torch.LongTensor(np.stack([self.connections2[_,:,:] for _ in left], axis=0))).cuda()
        ...
        right_connections2 = Variable(torch.LongTensor(np.stack([self.connections2[_,:,:] for _ in right], axis=0))).cuda()


        return (left_connections, left_degrees, right_connections, right_degrees, left_connections2,  right_connections2 )
